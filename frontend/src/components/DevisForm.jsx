...
const DevisForm = () => {
  ...
  const handleDevisNavigation = (direction) => {
    ...
    // Nettoyer les espaces éventuels dans le numéro de devis
    const cleanedNumbl = newNumbl.trim();
    localStorage.setItem("selectedDevisNumbl", cleanedNumbl);
  ...
  const validateNewMode = async (event) => {
    ...
    try {
      ...
      const { devis, articles } = response.data;

      setFormData({
        NUMBL: devis?.NUMBL || "",
        CODECLI: devis?.CODECLI || "",
        RSCLI: devis?.RSCLI || "",
        CODEREP: devis?.CODEREP || "",
        RSREP: devis?.RSREP || "",
        comm: devis?.comm || "",
        taxe: devis?.taxe || "", // Assure-toi que la taxe est bien intégrée
        lignes: Array.isArray(articles) ? articles : [],
      });
      ...
      const formattedTime = `${now.getHours().toString().padStart(2, "0")}:${now
        ...
        .toString()
        .padStart(2, "0")}`;
      setMlettreMessage(`${response.data.mlettre} - Créé à ${formattedTime}`);
    } catch (error) {
    ...
    <div>
      {!showDevis ? (
        ...
        >
          ...
          {mlettreMessage && (
            <p className="text-green-600 font-medium italic mt-2">
              {mlettreMessage}
            </p>
          )}
                    ...
                    {isNewMode || formData.RSCLI === "" ? (
                      <Select
                        ...
                        value={clientOptionsByRsoc.find(
                          (option) => option.value === formData.RSCLI
                        )}
